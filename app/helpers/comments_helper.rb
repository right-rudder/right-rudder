module CommentsHelper
  ...
  def user_profile_image(user)
    if user.profile_image.attached?
      image_tag url_for(user.resized_profile_image), class: "size-14 rounded-full"
    else
      content_tag :svg, class: "size-14 rounded-full bg-gray-50", viewBox: "0 0 24 24", fill: "currentColor" do
